...
public class fClearProfile : Form
{
  ...
  private void btnAdd_Click(object sender, EventArgs e)
  {
    ...
    int int_2 = 0;
    this.method_1(string.Format(GClass35.smethod_0("Đang dọn dẹp data {0}/{1}..."), (object) int_2, (object) this.list_0.Count));
    int int_0 = 0;
    int int_1 = 20;
    new Thread((ThreadStart) (() =>
    {
      try
      {
        ...
        while (num < this.list_0.Count)
        {
          if (int_0 < int_1)
          {
            ...
            new Thread((ThreadStart) (() =>
            {
              try
              {
                string str3 = GClass13.smethod_0() + "\\profile\\" + this.list_0[int_3];
                if (File.Exists(str3 + ".tar.gz"))
                  File.Delete(str3 + ".tar.gz");
                string str4 = GClass13.smethod_0() + "\\device\\" + this.list_0[int_3];
                if (File.Exists(str4 + ".tar.gz"))
                  File.Delete(str4 + ".tar.gz");
        ...
        {
          this.Close();
          GClass14.smethod_0((object) GClass35.smethod_0("Đã dọn dẹp profile xong!"));
        }));
      }
      catch
      {
  ...
  protected override void OnLoad(EventArgs e) => Application.Idle += new EventHandler(this.method_2);

  private void method_2(object sender, EventArgs e)
  {
    Application.Idle -= new EventHandler(this.method_2);
    new Thread((ThreadStart) (() =>
    {
      try
      {
        List<string> source = new List<string>();
        foreach (FileInfo file in new DirectoryInfo(GClass13.smethod_0() + "\\profile").GetFiles("*.tar.gz"))
          source.Add(Path.GetFileNameWithoutExtension(file.FullName).Split('.')[0]);
        foreach (FileInfo file in new DirectoryInfo(GClass13.smethod_0() + "\\device").GetFiles("*.tar.gz"))
          source.Add(Path.GetFileNameWithoutExtension(file.FullName).Split('.')[0]);
        this.list_0 = source.Distinct<string>().ToList<string>().Except<string>((IEnumerable<string>) this.method_0()).ToList<string>();
        if (this.list_0.Count > 0)
        {
          this.Invoke((Delegate) (() =>
          {
            this.lblStatus.Text = string.Format(GClass35.smethod_0("Tìm thấy {0} data rác!"), (object) this.list_0.Count) + "\r\n" + GClass35.smethod_0("Bạn có muốn dọn dẹp?");
        ...
        {
          GClass14.smethod_0((object) GClass35.smethod_0("Không có data rác!"));
  ...
  private void InitializeComponent()
  {
    ...
    ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof (fClearProfile));
    ...
    this.Name = nameof (fClearProfile);
    this.StartPosition = FormStartPosition.CenterScreen;
    this.Text = "fAddFile";
    this.Load += new EventHandler(this.fClearProfile_Load);
